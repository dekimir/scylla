...
BOOST_AUTO_TEST_CASE(parse_standard_syntax_error) {
    ...
    REQUIRE_EXCEPTION(cql_duration("1y500"), cql_duration_error, "Unable to convert '1y500' to a duration");

    // Do not skip invalid characters in the middle.
    REQUIRE_EXCEPTION(cql_duration("1y  xxx  500d"), cql_duration_error,
                      "Unable to convert '1y  xxx  500d' to a duration");

    // Do not skip invalid characters at the beginning.
    REQUIRE_EXCEPTION(cql_duration("xxx1y500d"), cql_duration_error, "Unable to convert 'xxx1y500d' to a duration");
}

BOOST_AUTO_TEST_CASE(parse_standard_order_error) {
    REQUIRE_EXCEPTION(cql_duration("20s1h3m"), cql_duration_error,
                      "Invalid duration. The seconds should be after hours");
}

BOOST_AUTO_TEST_CASE(parse_standard_repeated_error) {
    REQUIRE_EXCEPTION(cql_duration("1h2h3m"), cql_duration_error,
                      "Invalid duration. The hours are specified multiple times");
}

BOOST_AUTO_TEST_CASE(parse_standard_overflow_error) {
    REQUIRE_EXCEPTION(cql_duration("178956971y"), cql_duration_error,
                      "Invalid duration. The number of years must be less than or equal to 178956970");

    REQUIRE_EXCEPTION(cql_duration("178956970y14mo"), cql_duration_error,
...
BOOST_AUTO_TEST_CASE(parse_iso8601_syntax_error) {
  REQUIRE_EXCEPTION(cql_duration("P2003T23s"), cql_duration_error, "Unable to convert 'P2003T23s' to a duration");
}
...
BOOST_AUTO_TEST_CASE(parse_iso8601_alternative_syntax_error) {
  REQUIRE_EXCEPTION(cql_duration("P0001-00-02T000000"), cql_duration_error,
                    "Unable to convert 'P0001-00-02T000000' to a duration");
}

BOOST_AUTO_TEST_CASE(parse_component_overflow) {
  REQUIRE_EXCEPTION(cql_duration("10000000000000000000000000000000000m"), cql_duration_error,
                    "Invalid duration. The count for the minutes is out of range");

  REQUIRE_EXCEPTION(cql_duration("P10000000000000000000000000000000000Y5D"), cql_duration_error,
                    "Invalid duration. The count for the years is out of range");
}
