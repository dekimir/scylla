...
SEASTAR_TEST_CASE(slice_multi_column) {
    return do_with_cql_env_thread([](cql_test_env& e) {
        ...
        BOOST_CHECK_EQUAL(slice_parse("(c1)<(1)", e), std::vector{left_closed_right_open({}, {I(1)})});
        // TODO: Uncomment when supported:
        ...
        BOOST_CHECK_EQUAL(slice_parse("(c1,c2)>=(1,2)", e), std::vector{both_closed({I(1), I(2)}, {})});
        ...
        BOOST_CHECK_EQUAL(slice_parse("(c1,c2,c3)>(1,2,3)", e), std::vector{left_open_right_closed({I(1), I(2), I(3)}, {})});
