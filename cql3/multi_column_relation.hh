...
protected:
    virtual shared_ptr<restrictions::restriction> new_EQ_restriction(database& db, schema_ptr schema,
        ...
        restr->wip_equivalent = ::make_shared<expression>(binary_operator{rs, _relation_type, t});
        return restr;
    }
    ...
    virtual shared_ptr<restrictions::restriction> new_IN_restriction(database& db, schema_ptr schema,
        ...
        if (_in_marker) {
            ...
            if (ts.size() == 1) {
                ...
                using namespace restrictions::wip;
                restr->wip_equivalent = ::make_shared<expression>(binary_operator{rs, operator_type::EQ, ts[0]});
                return restr;
            }
