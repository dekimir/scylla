...
BOOST_AUTO_TEST_CASE(test_duration_type_validation) {
    ...
    REQUIRE_EXCEPTION(duration_type->validate(from_hex("ff")), marshal_exception,
        "marshaling error: Expected at least 3 bytes for a duration, got 1");

    REQUIRE_EXCEPTION(duration_type->validate(from_hex("fffffffffffffffffe0202")), marshal_exception,
        "marshaling error: The duration months (9223372036854775807) must be a 32 bit integer");

    REQUIRE_EXCEPTION(duration_type->validate(from_hex("010201")), marshal_exception,
