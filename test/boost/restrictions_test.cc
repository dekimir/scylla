...
SEASTAR_THREAD_TEST_CASE(tuple_of_list) {
    do_with_cql_env_thread([](cql_test_env& e) {
        ...
        wip_require_rows(e, "select l1 from t where (l1,l2)<([20],[200]) allow filtering",
                         {{LI({11, 12}), LI({101, 102})}});
...
SEASTAR_THREAD_TEST_CASE(multi_col_slice) {
    do_with_cql_env_thread([](cql_test_env& e) {
        ...
        wip_require_rows(e, "select c2 from t where (c1,c2)>('a',20) allow filtering",
                         {{F(2), T("b")}, {F(13), T("c")}});
...
SEASTAR_THREAD_TEST_CASE(set_contains) {
    do_with_cql_env_thread([](cql_test_env& e) {
        cquery_nofail(e, "create table t (p frozen<set<int>>, c frozen<set<int>>, s set<text>, st set<int> static,"
                      " primary key (p, c))");
...
SEASTAR_THREAD_TEST_CASE(list_contains) {
    do_with_cql_env_thread([](cql_test_env& e) {
        ...
        wip_require_rows(e, "select c from t where c contains 21 allow filtering",
                         {{LI({21,22,23})}, {LI({21,32,33})}});
